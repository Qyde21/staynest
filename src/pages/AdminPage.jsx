          ...
          <div className="admin-section">
            ...
            <div className="admin-form">
              ...
              <div className="admin-form__field">
                ...
                <textarea rows={4} value={newProperty.description} onChange={(e) => setNewProperty({...newProperty, description: e.target.value})} placeholder="Describe the property..." />
              </div>
              <div className="admin-form__field">
                ...
                <tbody>
                  {bookings.map((b) => (
                    <tr key={b.id}>
                      ...
                      <td>
                        {b.status === 'confirmed' && (
                          <button className="admin-action-btn admin-action-btn--danger" onClick={() => handleCancelBooking(b.id)}>
                            Cancel
                          </button>
                        )}
                      </td>
                ...
                <tbody>
                  {users.map((u) => (
                    <tr key={u.id}>
                      ...
                      <td>{u.phone || '-'}</td>
                      <td>
                        {u.is_admin ? <span className="admin-badge admin-badge--paid">Admin</span>
                          : u.is_host ? <span className="admin-badge admin-badge--confirmed">Host</span>
                          : <span className="admin-badge">Guest</span>}
                      </td>
                      <td>{new Date(u.created_at).toLocaleDateString('en-KE')}</td>
                      <td>
                        {!u.is_admin && (
                          <button className="admin-action-btn admin-action-btn--danger" onClick={() => handleDeleteUser(u.id)}>
                            Delete
                          </button>
                        )}
                      </td>
                ...
                <tbody>
                  {reviews.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        <button className="admin-action-btn admin-action-btn--danger" onClick={() => handleDeleteReview(r.id)}>
                          Delete
                        </button>
                      </td>
                    </tr>
